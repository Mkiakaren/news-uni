...
if ($_POST && $action == 'add') {
    ...
    if (empty($title) || empty($content) || empty($category_id)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO news (title, slug, summary, content, image_url, author_id, category_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'approved')");
...
if ($_POST && $action == 'edit') {
    ...
    if (empty($title) || empty($content) || empty($category_id)) {
        ...
        $stmt = $pdo->prepare("UPDATE news SET title = ?, slug = ?, summary = ?, content = ?, image_url = ?, category_id = ? WHERE id = ?");
        if ($stmt->execute([$title, $slug, $summary, $content, $image_url, $category_id, $newsId])) {
...
<head>
    ...
    <title>مدیریت اخبار - پنل مدیریت</title>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <a href="?toggle_status=<?= $item['id'] ?>&status=<?= $item['status'] ?>" class="btn btn-sm btn-info"
                                                       onclick="return confirm('آیا از تغییر وضعیت این خبر اطمینان دارید؟')">
                        ...
                        <div class="card-body">
                            <form method="POST">
                                ...
                                <div class="alert alert-info">
                                    <i class="fas fa-info-circle"></i> Slug به صورت خودکار از عنوان خبر تولید می‌شود.
                                </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <form method="POST">
                                ...
                                <div class="alert alert-info">
                                    <i class="fas fa-info-circle"></i> Slug فعلی: <code><?= $editNews['slug'] ?></code><br>
                                    در صورت تغییر عنوان، Slug جدید تولید می‌شود.
                                </div>
